...
%%
%Finding the distances for every specific point
...
for n=1:length(N)
    for r=1:length(R)
        for teta=1:length(theta)
            for i=1:length(delayedPulse)-SampleArr(7,8,n)
                lastPulse(n,i,r,teta)=delayedPulse(n,i+SampleArr(7,8,n),r,teta);
            end
        end
    end
end 
...
fig1=figure('Name','Real Part');
fig2=figure('Name','Imaginary Part');
figure(fig1)
mesh(pixel_real)
xlabel('Theta')
ylabel('R')
